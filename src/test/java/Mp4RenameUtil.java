 ...
 * 7. deleteFileByStartEnd   【新增】删除同时满足以start开头、end结尾的文件，至少传一个条件
 */
public class Mp4RenameUtil {
    /**
     * 1. mp4文件名删除指定字符串重命名
    ...
    public static void deleteDotStartMp4File(String rootDirPath, boolean isTest) {
        // 【可选改造】旧方法直接调用新增方法，减少重复代码
        deleteFileByStartEnd(rootDirPath, ".", ".mp4", isTest);
    }

    /**
     * 【新增7号方法】删除文件名满足【start开头】并且【end结尾】的文件
     * <p>至少传入start或end其中一个参数，不能两个同时为空</p>
     * <ul>
     * <li>start非空：文件名必须 start.startsWith(start)</li>
     * <li>end非空：文件名必须 end.endsWith(end)</li>
     * <li>两个都传：两个条件同时满足才匹配删除</li>
     * </ul>
     * @param rootDirPath 主目录
     * @param start 文件开头匹配字符串，可以传null/空字符串代表不校验开头
     * @param end 文件结尾匹配字符串，可以传null/空字符串代表不校验结尾
     * @param isTest true=预览，false=真实执行
     */
    public static void deleteFileByStartEnd(String rootDirPath, String start, String end, boolean isTest) {
        File rootDir = new File(rootDirPath);
        if (!rootDir.exists() || !rootDir.isDirectory()) {
            System.err.println("目录不存在：" + rootDirPath);
            return;
        }
        // 参数校验：start、end不能同时为空
        boolean startEmpty = (start == null || start.isEmpty());
        boolean endEmpty = (end == null || end.isEmpty());
        if (startEmpty && endEmpty) {
            System.err.println("参数错误：start和end不能同时为空，至少提供一个匹配条件");
            return;
        }
        walkDirDeleteByStartEnd(rootDir, start, end, isTest);
    }

    private static void walkDirDeleteByStartEnd(File currentDir, String start, String end, boolean isTest) {
        File[] files = currentDir.listFiles();
        if (files == null) {
            return;
        }
        for (File file : files) {
            if (file.isDirectory()) {
                walkDirDeleteByStartEnd(file, start, end, isTest);
            } else {
                String fileName = file.getName();
                boolean match = true;
                ...
                if (match) {
                    ...
                    if (deleteOk) {
                        System.out.printf("【首尾条件匹配已删除】%s%n", file.getAbsolutePath());
                    } else {
                        System.err.printf("【首尾条件匹配删除失败】%s，文件可能被占用/权限不足%n", file.getAbsolutePath());
                    }
                }
            }
        }
    ...
    private static void walkDirDeleteDotMp4(File currentDir, boolean isTest) {
        // 旧递归方法已经废弃，因为deleteDotStartMp4File已经委托给新方法，保留空方法防止外部调用报错，也可以直接删掉
    }
    ...
    public static void main(String[] args) {
        String rootPath = "G:\\study\\Cursor\\Cursor入门到实战 - 副本";
        boolean isTest = true; // true预览模式，false真实执行

        // 1. 删除mp4文件名中出现的指定字符串
        //renameMp4File(rootPath, "AI", false);

        // 2.删除点开头mp4    .123.mp4
        //deleteDotStartMp4File(rootPath, false);
        // 同时匹配开头+结尾：以cache开头、.zip结尾
        //deleteFileByStartEnd(rootPath, ".", "", true);
        ...
        //5.字符串替换
        //replaceFileNameStr(rootPath, ".mp", ".mp4", isTest);

        //6.正则删除片段
        //removeFileNameByRegex(rootPath, "\\d+", false);
